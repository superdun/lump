...
class Example(Frame):
    ...
    def bestUI(self):
        ...
        self.GASOLvar = IntVar()
        self.GASOLc = Checkbutton(frame2, text='GASOL', variable=self.GASOLvar)
        self.GASOLc.grid(row=4, column=9, columnspan=1, rowspan=1, sticky=W, pady=4, padx=5)
    ...
    def cateUI(self):
        ...
        # HS,HA,HR,DIESEL,GASOL,DGAS,LO3,LO4,LPGD,COKE
        lbl = Label(frame2, text="HS")
        lbl.grid(row=1, column=4, columnspan=2, rowspan=1, sticky=W, pady=4, padx=5)
        ...
        lbl = Label(frame2, text="GASOL")
        lbl.grid(row=5, column=4, columnspan=2, rowspan=1, sticky=W, pady=4, padx=5)
        self.GASOL = Entry(frame2)
        self.GASOL.grid(row=5, column=6, columnspan=2, rowspan=1, sticky=W, pady=4, padx=5)
    ...
    def graphUI(self):
        ...
        # self.chartResultName.insert(0, 'HS,HA,HR,DIESEL,GASOL,DGAS,LO3,LO4,LPGD,COKE')
        ...
        self.GASOLvar = IntVar()
        self.GASOLc = Checkbutton(frame2, text='GASOL', variable=self.GASOLvar)
        self.GASOLc.grid(row=4, column=14, columnspan=1, rowspan=1, sticky=W, pady=4, padx=5)
    ...
    def doPre(self):
        ...
        Y0_raw = [self.HS.get(),self.HA.get(),self.HR.get(),0, 0, 0, 0, 0, 0, 0]
    ...
    def doBest(self):
        ...
        n = catObj.n
        resultIdArray = []
        chartResultIdArray = [self.HSvar,self.HAvar,self.HRvar,self.DIESELvar,self.GASOLvar,self.LO3var,self.LPGDvar,self.COKEvar]
    ...
    def doChart(self):
        ...
        resultName = 'HS,HA,HR,DIESEL,GASOL,LO3,LPGD,COKE'
    ...
    def importExcel(self):
        ...
        if fl != '':
            ...
            resultArray = excelModule.getFactorsFromExcel(sheet,t_resid,w_nitro,w_aro,Y0_raw)
            for i in resultArray:
                # HS,HA,HR,DIESEL,GASOL,LO3,LPGD,COKE
                p = i['p']
                Y0 = numpy.mat(Y0_raw).astype(numpy.float)
                ...
                self.Molmasses = numpy.mat(
                    [0.8, 1.1, 1.8, 0.2, 0.11,  0.042,  0.05, 0.012]).astype(numpy.float)
                ...
                self.factorMonitor.insert('end',
                                          u"HS：%s,HA：%s,HR：%s,DIESEL：%s,GASOL：%s,LO3：%s,LPGD：%s,COKE：%s\n"
    ...
    def addFactors(self):
        ...
        Y0_raw = [self.HS.get(),self.HA.get(),self.HR.get(),0, 0, 0, 0, 0, 0, 0]
        Y0 = numpy.mat(Y0_raw).astype(numpy.float)
        Y_results_raw =  [self.HSy.get(),self.HAy.get(),self.HRy.get(),self.DIESEL.get(),self.GASOL.get(),self.LO3.get(),self.LPGD.get(),self.COKE.get()]
        Y_results = numpy.mat(Y_results_raw).astype(numpy.float)
        w_aro = float(self.HA.get())
        w_nitro = float(self.yn_input.get())
        t = float(self.T_input.get())
        r_oil = float(self.roil_input.get())
        self.Molmasses = numpy.mat([0.8, 1.1, 1.8, 0.2, 0.11,  0.042,  0.05, 0.012]).astype(numpy.float)
        self.factorMonitor.insert( 'end', "#############################################\n")
        self.factorMonitor.insert('end', u"温度：%f,压力：%f,停留时间：%f,碱氮：%f\n"%(t,p,t_resid,w_nitro))
        self.factorMonitor.insert('end', u"初始组成：\n")
        self.factorMonitor.insert('end', u"HS：%s,HA：%s,HR：%s\n"%(self.HS.get(),self.HA.get(),self.HR.get()))
        self.factorMonitor.insert('end', u"实际产率：\n")
        self.factorMonitor.insert('end', u"HS：%s,HA：%s,HR：%s,DIESEL：%s,GASOL：%s,LO3：%s,LPGD：%s,COKE：%s\n"
                                  % (self.HSy.get(), self.HAy.get(), self.HRy.get(), self.DIESEL.get(), self.GASOL.get(),
                                     self.LO3.get(), self.LPGD.get(),self.COKE.get())
                                  )
        if self.catFactors.has_key(t):
            self.catFactors[t].append(
    ...
    def makePreResultUI(self, target, result):
        ...
        danteng = ['HS','HA','HR','DIESEL','GASOL','LO3','LPGD','COKE']
